...
export function LobbyView({ gameState, roomId, emit }: LobbyViewProps) {
  const router = useRouter();
  const userId = typeof window !== 'undefined' ? localStorage.getItem('clue_userId') : '';

  const handleStartGame = () => {
    emit(SocketEvents.START_GAME, { roomId, userId }, (result: any) => {
      if (!result.success) {
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-900 to-black p-6">
      <div className="max-w-2xl mx-auto">
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-purple-400 mb-2">Room: {roomId}</h1>
